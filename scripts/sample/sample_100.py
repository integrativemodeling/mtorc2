...
def run_86(
    ...
    # EM DISTANCE RESTRAINTS
    if kappa > 0:
        # Kinase C lobe.
        em_dr_0 = IMP.pmi.restraints.basic.DistanceToPointRestraint(
            root_hier=s.get_hierarchy(),
            tuple_selection=(189, 189, "KINASE", 0),
            anchor_point=IMP.algebra.Vector3D(121.382, 58.031, 182.312),
            radius=radius,
            kappa=kappa
        )
        em_dr_0.add_to_model()
        output_objects.append(em_dr_0)

        em_dr_1 = IMP.pmi.restraints.basic.DistanceToPointRestraint(
            root_hier=s.get_hierarchy(),
            tuple_selection=(214, 214, "KINASE", 0),
            anchor_point=IMP.algebra.Vector3D(127.850,  73.835, 179.762),
            radius=radius,
            kappa=kappa
        )
        em_dr_1.add_to_model()
        output_objects.append(em_dr_1)

        em_dr_2 = IMP.pmi.restraints.basic.DistanceToPointRestraint(
            root_hier=s.get_hierarchy(),
            tuple_selection=(114, 114, "KINASE", 0),
            anchor_point=IMP.algebra.Vector3D(113.434,  46.674, 185.923),
            radius=radius,
            kappa=kappa
        )
        ...
        # Kinase N lobe.
        em_dr_3 = IMP.pmi.restraints.basic.DistanceToPointRestraint(
            root_hier=s.get_hierarchy(),
            tuple_selection=(33, 33, "KINASE", 0),
            anchor_point=IMP.algebra.Vector3D(141.878,  45.228, 174.685),
            radius=radius,
            kappa=kappa
        )
        ...
        em_dr_4 = IMP.pmi.restraints.basic.DistanceToPointRestraint(
            root_hier=s.get_hierarchy(),
            tuple_selection=(45, 45, "KINASE", 0),
            anchor_point=IMP.algebra.Vector3D(148.382 , 63.606, 185.632),
            radius=radius,
            kappa=kappa
        )
        em_dr_4.add_to_model()
        output_objects.append(em_dr_4)

        em_dr_5 = IMP.pmi.restraints.basic.DistanceToPointRestraint(
            root_hier=s.get_hierarchy(),
            tuple_selection=(80, 80, "KINASE", 0),
            anchor_point=IMP.algebra.Vector3D(140.952 , 46.804, 179.902),
            radius=radius,
            kappa=kappa
        )
        em_dr_5.add_to_model()
        output_objects.append(em_dr_5)
